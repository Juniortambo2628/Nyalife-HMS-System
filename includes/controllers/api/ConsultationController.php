...
class ConsultationController extends ApiController {
    ...
    public function saveConsultation() {
        ...
        try {
            ...
            $treatment = isset($data['treatment']) ? trim($data['treatment']) : '';
            $healthEducation = isset($data['health_education']) ? trim($data['health_education']) : '';
            ...
            if (isset($data['vital_signs']) && is_array($data['vital_signs'])) {
                ...
                $temperature = isset($vitalSigns['temperature']) ? $vitalSigns['temperature'] : null;
                $bloodPressure = isset($vitalSigns['blood_pressure']) ? $vitalSigns['blood_pressure'] : null;
                $pulseRate = isset($vitalSigns['pulse_rate']) ? $vitalSigns['pulse_rate'] : null;
                $respiratoryRate = isset($vitalSigns['respiratory_rate']) ? $vitalSigns['respiratory_rate'] : null;
                $height = isset($vitalSigns['height']) ? $vitalSigns['height'] : null;
                $weight = isset($vitalSigns['weight']) ? $vitalSigns['weight'] : null;
                $bmi = isset($vitalSigns['bmi']) ? $vitalSigns['bmi'] : null;
                $spo2 = isset($vitalSigns['spo2']) ? $vitalSigns['spo2'] : null;
        ...
        try {
            ...
            foreach ($medications as $medication) {
                ...
                if (!isset($medication['medication_id']) || !isset($medication['dosage']) || 
                    !isset($medication['frequency']) || !isset($medication['duration'])) {
                    continue;
                }
                ...
                $instructions = isset($medication['instructions']) ? trim($medication['instructions']) : '';
    ...
    public function saveLabRequest() {
        // Only doctors can request lab tests
        if ($this->userRole !== 'doctor' && $this->userRole !== 'admin') {
            $this->sendError('Only doctors can request lab tests', 403);
            return;
        }
        ...
        // Validate required fields
        $requiredFields = ['consultation_id', 'patient_id', 'test_types'];
        ...
        try {
            ...
            // Validate test types array
            if (!is_array($testTypes) || empty($testTypes)) {
                $this->sendError('No test types specified');
                return;
            }
            
            // Validate priority
            $validPriorities = ['routine', 'urgent', 'stat'];
            ...
            // Create lab request
            $query = "INSERT INTO lab_requests (
                    consultation_id,
                    patient_id,
                    doctor_id,
                    priority,
            ...
            $query = "INSERT INTO lab_request_items (
                    request_id,
                    test_type_id,
                    status
                ) VALUES (?, ?, 'pending')";
     ...
     * @return void
     */
    public function deleteLabRequest() {
    ...
    public function deleteReferral() {
        ...
        try {
            ...
            $referral = $this->fetchOne(
                "SELECT r.*, c.consultation_status
                FROM referrals r
                JOIN consultations c ON r.consultation_id = c.consultation_id
                WHERE r.referral_id = ?",
                [$referralId]
            ...
            $result = $this->execute($sql, [$referralId]);
            ...
            $this->execute($auditSQL, [$this->userId, $referralId, $auditDescription, $ipAddress]);
    ...
    public function deleteFollowup() {
        ...
        try {
            ...
            $followup = $this->fetchOne(
                "SELECT f.*, c.consultation_status
                FROM follow_ups f
    ...
    public function saveReferral() {
        ...
        try {
            ...
            if ($referralId > 0) {
                ...
                // Bind parameters
                $types = str_repeat('s', count($referralData));
                $stmt->bind_param($types, ...array_values($referralData));
     ...
     */
    private function checkConsultationAccess($consultationId) {
