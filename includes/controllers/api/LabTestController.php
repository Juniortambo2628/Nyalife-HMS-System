...
require_once __DIR__ . '/../../../includes/data/lab_data.php';
...
class LabTestController extends ApiController {
    ...
    public function getTestResult() {
        ...
        try {
            ...
            $query = "SELECT p.parameter_name,
                 ...
                 LEFT JOIN lab_test_results r ON p.parameter_id = r.parameter_id 
                     AND r.test_item_id = ?
                 WHERE p.test_id = ?
     ...
     */
    private function checkTestAccessPermission($test) {
        // Admin can view all
     ...
     */
    private function generateTestResultHtml($test, $parameters) {
        $html = '
        <div class="container-fluid">
            <div class="row mb-4">
                <div class="col-md-6">
                    ...
                    <p>
                        <strong>Name:</strong> ' . htmlspecialchars($test['patient_name']) . '<br>
                        <strong>Patient ID:</strong> ' . htmlspecialchars($test['patient_number']) . '
                    </p>
                </div>
                <div class="col-md-6">
                    ...
                    <p>
                        ...
                        <strong>Requested By:</strong> Dr. ' . htmlspecialchars($test['doctor_name']) . '<br>
        ...
        foreach ($parameters as $param) {
            ...
            if ($result_value !== 'Pending' && $param['reference_range']) {
                $range = explode('-', $param['reference_range']);
                if (count($range) === 2) {
            ...
            $html .= '
                <tr>
                    ...
                    <td class="' . ($is_normal ? 'text-success' : 'text-danger') . '">' . 
                        htmlspecialchars($result_value) . '</td>
                    <td>' . htmlspecialchars($param['unit']) . '</td>
        ...
        if (!empty($test['test_description'])) {
            ...
            <div class="row mt-4">
                <div class="col-12">
                    ...
                    <p>' . nl2br(htmlspecialchars($test['test_description'])) . '</p>
                </div>
            </div>';
        }
        ...
        if (!empty($test['notes'])) {
            ...
            <div class="row mt-4">
                <div class="col-12">
                    ...
                    <p>' . nl2br(htmlspecialchars($test['notes'])) . '</p>
                </div>
            </div>';
        }
     ...
     */
    private function userHasPermissionForLabRequest($request) {
