...
{
    $result = selectSingle("SELECT COUNT(*) as count FROM patients");
 ...
 */
function createPatient(array $userData, $patientData)
{
    try {
 ...
 */
function updatePatient($patientId, $userData = [], $patientData = []): bool
{
    try {
        $db = dbBeginTransaction();
...
function getActivePatientPrescriptions($patientId)
{
    ...
    return selectQuery($sql, [$patientId]);
}
...
function getPatientPrescriptionItems($prescriptionId)
{
    ...
    return selectQuery($sql, [$prescriptionId]);
}
