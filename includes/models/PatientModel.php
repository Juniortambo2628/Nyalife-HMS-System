...
class PatientModel extends BaseModel {
    ...
    public function getMedicalHistory($patientId) {
        try {
            $sql = "SELECT mh.*, u.first_name, u.last_name
                    ...
                    JOIN users u ON mh.created_by = u.user_id
                    WHERE mh.patient_id = ?
    ...
    public function createPatient($userData, $patientData) {
        try {
            ...
            if (!isset($patientData['registration_date'])) {
                $patientData['registration_date'] = date('Y-m-d');
            }
     ...
     */
    public function updatePatient($patientId, $userData = [], $patientData = []) {
        try {
            // Start transaction
            $this->db->begin_transaction();
     ...
     */
    public function getPatientCountByDoctor($doctorId) {
        try {
