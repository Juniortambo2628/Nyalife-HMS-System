...
class PrescriptionModel extends BaseModel
{
     ...
     */
    public function createPrescription($data): int|string|false
    {
        try {
            // Begin transaction
            ...
            $prescriptionSql = "INSERT INTO prescriptions (
                prescription_number, patient_id, created_by, appointment_id, 
            ...
            if (isset($data['items']) && is_array($data['items']) && (isset($data['items']) && $data['items'] !== [])) {
    ...
    public function getPrescriptionWithItems($prescriptionId)
    {
        try {
            ...
            $prescriptionSql = "SELECT p.*,
                           ...
                           JOIN users doc_user ON p.prescribed_by = doc_user.user_id
                           JOIN users created_user ON p.prescribed_by = created_user.user_id
     ...
     */
    public function getAllPrescriptions(array $filters = [])
    {
        try {
            $sql = "SELECT p.*,
                          CONCAT(pat_user.first_name, ' ', pat_user.last_name) as patient_name,
    ...
    public function getCompletedPrescriptions($limit = null)
    {
        try {
            $sql = "SELECT p.*,
                   ...
                   JOIN patients pat ON p.patient_id = pat.patient_id
                   JOIN users pat_user ON pat.user_id = pat_user.user_id
                   JOIN users doc_user ON p.doctor_id = doc_user.user_id
                   LEFT JOIN users disp_user ON p.dispensed_by = disp_user.user_id
    ...
    {
        try {
            $sql = "SELECT pi.*, m.medication_name, m.medication_type, m.unit, m.strength
                    FROM prescription_items pi
                    JOIN medications m ON pi.medication_id = m.medication_id
                    WHERE pi.prescription_id = ?
                    ORDER BY pi.item_id ASC";
     ...
     * @return bool Success status
     */
    public function dispensePrescription($prescriptionId, $dispensedBy, $dispensedItems = []): bool
    {
        try {
            $this->db->begin_transaction();
     ...
     *
     * @param int $doctorId Doctor ID (user_id of the doctor)
     ...
     * @return array Array of prescriptions
     */
    public function getPrescriptionsByDoctor($doctorId, $status = 'active', $patientId = 0, $page = 1, $perPage = 15)
    {
        try {
            $sql = "SELECT p.*,
                          CONCAT(pat_user.first_name, ' ', pat_user.last_name) as patient_name,
    ...
    /**
     * Count prescriptions by doctor with filtering
     *
     * @param int $doctorId Doctor ID (user_id of the doctor)
     * @param string $status Status filter
     * @param int $patientId Optional patient ID filter
     * @return int Count of prescriptions
     */
    public function countPrescriptionsByDoctor($doctorId, $status = 'active', $patientId = 0): int
    {
        try {
     ...
     * @return array Array of prescriptions
     */
    public function getPrescriptionsByStatus($status = 'active', $patientId = 0, $page = 1, $perPage = 15)
    {
        try {
            $sql = "SELECT p.*,
                          CONCAT(pat_user.first_name, ' ', pat_user.last_name) as patient_name,
            ...
            $stmt = $this->db->prepare($sql);
            // $params always has at least $offset and $perPage, so always bind
            $stmt->bind_param($types, ...$params);
            $stmt->execute();
    ...
    /**
     * Count prescriptions with filtering
     *
     * @param string $status Status filter
     * @param int $patientId Optional patient ID filter
     * @return int Count of prescriptions
     */
    public function countPrescriptions($status = 'active', $patientId = 0): int
    {
        try {
            $sql = "SELECT COUNT(*) as count
                   FROM prescriptions
