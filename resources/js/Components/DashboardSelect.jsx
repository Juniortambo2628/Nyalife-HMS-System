...
export default function DashboardSelect({
    ...
    const filteredOptions = useMemo(() => {
        ...
        return options.filter((opt) => String(opt[labelField]).toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    const triggerBgColor =
        isOpen || value ? 'bg-white' : isDark ? 'bg-white bg-opacity-20 hover-bg-opacity-30' : 'bg-white';

    const hasValue = !!value;

    return (
        <div
            className={`dashboard-select-container position-relative ${isOpen ? 'is-open' : ''} ${hasValue ? 'has-value' : ''} ${className}`}
            ref={dropdownRef}
            style={style}
        >
            {/* Trigger Area */}
            ...
            >
                <div className="d-flex align-items-center gap-2 overflow-hidden flex-grow-1">
                    {loading ? (
                        <div
                            className="spinner-border spinner-border-sm text-pink-500 opacity-50 nyl-select-spinner"
                            role="status"
                        ></div>
                    ) : (
                ...
                <div className="d-flex align-items-center gap-2">
                    ...
                    <i
                        className={`fas fa-chevron-${isOpen ? (dropup ? 'up' : 'down') : dropup ? 'down' : 'up'} text-pink-500 fs-xs transition-all duration-500 ${isOpen ? 'opacity-100' : 'opacity-40'}`}
                    ></i>
                </div>
            </div>
            ...
            {isOpen && (
                <div
                    className={`position-absolute start-0 w-100 shadow-2xl overflow-hidden nyl-select-dropdown bg-white border border-gray-100 ${dropup ? 'bottom-100 mb-3' : 'top-100 mt-3'}`}
                    ...
                    <div className="overflow-auto custom-scrollbar" style={{ maxHeight: '300px' }}>
                        {loading && asyncUrl && (
                            <div className="p-4 text-center">
                                <div
                                    className="spinner-border text-primary spinner-border-sm opacity-25"
                                    role="status"
                                ></div>
                                <div className="mt-2 extra-small text-muted fw-extrabold text-uppercase tracking-widest">
                                    Scanning Catalog...
                                </div>
                            </div>
                        )}
                        ...
                        {!loading && filteredOptions.length > 0
                            ? filteredOptions.map((opt, i) => (
                                  ...
                                  >
                                      <div className="d-flex flex-column">
                                          <span className="fw-bold extra-small text-uppercase tracking-tight">
                                              {opt[labelField]}
                                          </span>
                                          {opt.sublabel && (
                                              <small className="text-muted extra-small opacity-75">
                                                  {opt.sublabel}
                                              </small>
                                          )}
                                      </div>
                            ...
                            : !loading && (
                                  <div className="p-5 text-center">
                                      <div
                                          className="bg-gray-50 rounded-circle d-inline-flex align-items-center justify-content-center mb-3"
                                          style={{ width: '60px', height: '60px' }}
                                      >
                                          <i className="fas fa-search-minus text-gray-200 fs-4"></i>
                                      </div>
                                      <p className="mb-0 fw-extrabold extra-small text-muted text-uppercase tracking-widest opacity-50">
