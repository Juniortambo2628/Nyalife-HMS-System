...
export default function DashboardTable({
                ...
                <table className="table table-hover align-middle mb-0 border-0">
                    <thead>
                        {table.getHeaderGroups().map((headerGroup) => (
                            <tr
                                key={headerGroup.id}
                                className={`nyl-table-head-row ${headerBgClassName} border-0 border-bottom`}
                            >
                                {headerGroup.headers.map((header) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div
                                                className={`d-flex align-items-center gap-2 ${header.id === 'selection' ? 'justify-content-center' : ''}`}
                                            >
                            ...
                            table.getRowModel().rows.map((row) => (
                                <tr key={row.id} className="nyl-table-row border-bottom border-light">
                                    {row.getVisibleCells().map((cell) => (
                                        <td
                                            key={cell.id}
                                            className={`nyl-table-td px-3 py-2 border-0 ${cell.column.id === 'selection' ? 'text-center' : 'text-start'} align-middle`}
                                        >
    ...
    if (noCard) {
        return <div className={`dashboard-table-wrapper ${className}`}>{tableContent}</div>;
    }

    return (
        <div className={`dashboard-table-wrapper ${className}`}>
            <div className="card shadow-sm border-0 rounded-2xl overflow-hidden bg-white">{tableContent}</div>
        </div>
    );
}
