    ...
    return (
        ...
        >
            ...
            <form onSubmit={submit} className="pb-5">
                            ...
                            <select 
                                ...
                                onChange={(e) => {
                                    ...
                                    setData(d => ({
                                        ...d,
                                        head_name: name,
                                        head_position: matchedUser ? (matchedUser.role.replace('_', ' ').charAt(0).toUpperCase() + matchedUser.role.replace('_', ' ').slice(1)) : d.head_position
                                    }));
                                }}
                            >
                                <option value="">Select Department Head...</option>
                                {assignedStaff.map(u => (
                                    <option key={u.user_id} value={u.name}>{u.name} ({u.role.replace('_', ' ')})</option>
                                ))}
                            </select>
                ...
                <FormSection title="Assign Staff Members" icon="fas fa-users" className="mt-4" headerClassName="bg-primary text-white p-3">
                    <div className="row g-3">
                        <div className="col-12">
                            <label className="form-label font-bold text-gray-500 mb-3">Select the staff members to assign to this department:</label>
                            <div className="row g-3">
                                {staffUsers.length > 0 ? (
                                    staffUsers.map(u => {
                                        const isChecked = data.assigned_user_ids.includes(u.user_id);
                                        const isAssignedElsewhere = u.department_id && u.department_id !== department?.department_id;
                                        return (
                                            <div key={u.user_id} className="col-md-6 col-lg-4">
                                                <div className={`card p-3 border rounded-3 bg-white h-100 transition-all ${isChecked ? 'border-primary shadow-sm bg-light' : ''}`}>
                                                            ...
                                                            onChange={(e) => {
                                                                ...
                                                                setData(d => {
                                                                    const ids = checked 
                                                                        ...
                                                                        : d.assigned_user_ids.filter(id => id !== u.user_id);
                                                                    // If unchecked user was the head, clear head_name
                                                                    const headName = d.head_name === u.name && !checked ? '' : d.head_name;
                                                                    return { ...d, assigned_user_ids: ids, head_name: headName };
                                                                });
                                                            }}
                                                        />
                                                        <label className="form-check-label d-flex flex-column" htmlFor={`user-${u.user_id}`} style={{ cursor: 'pointer' }}>
                                                            <span className="fw-bold text-gray-800">{u.name}</span>
                                                            <span className="extra-small text-muted text-uppercase fw-semibold" style={{ fontSize: '0.75rem' }}>{u.role.replace('_', ' ')}</span>
                                                            {isAssignedElsewhere && (
                                                                <span className="extra-small text-warning font-bold mt-1" style={{ fontSize: '0.7rem' }}>
                                                                    <i className="fas fa-exclamation-triangle me-1"></i> Currently in another department
                                                                </span>
                                                            )}
                                                        </label>
                                                    </div>
