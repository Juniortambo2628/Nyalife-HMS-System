    ...
    return (
        ...
        >
                    ...
                    <div className="card border-0 shadow-sm rounded-4 bg-white">
                        <div className="card-body p-5">
                            <div className="d-flex justify-content-between align-items-start mb-4">
                                <div>
                                    <div className="extra-small fw-bold text-muted text-uppercase tracking-widest mb-1">Scheduled Date</div>
                            ...
                            <div className="row g-4 mb-4">
                                <div className="col-md-6">
                                    <div className="p-4 rounded-4 bg-gray-50 border">
                                        <h6 className="extra-small fw-extrabold text-muted text-uppercase mb-2">Patient</h6>
                                        <div className="fw-bold">{followUp.patient?.user?.first_name} {followUp.patient?.user?.last_name}</div>
                                    </div>
                                </div>
                                <div className="col-md-6">
                                    <div className="p-4 rounded-4 bg-gray-50 border">
                                        <h6 className="extra-small fw-extrabold text-muted text-uppercase mb-2">Type</h6>
                    ...
                    {followUp.consultation_id && (
                        <div className="card border-0 shadow-sm rounded-4 mb-4">
                            <div className="card-body p-4">
                                <h6 className="fw-extrabold text-muted extra-small text-uppercase tracking-widest mb-3">Linked Consultation</h6>
                                <Link href={route('consultations.show', { consultation: followUp.consultation_id })} className="btn btn-outline-primary w-100 rounded-pill fw-bold">
            ...
            {canManage && (
                <UnifiedToolbar
                    actions={[
                        followUp.status === 'scheduled' && { label: 'MARK COMPLETED', icon: 'fa-check-circle', onClick: () => updateStatus('completed'), color: 'success' },
                        followUp.status === 'scheduled' && { label: 'NO SHOW', icon: 'fa-user-times', onClick: () => updateStatus('no_show'), color: 'warning' },
                        followUp.status === 'scheduled' && { label: 'CANCEL', icon: 'fa-ban', onClick: () => updateStatus('cancelled'), color: 'danger' },
                        { label: 'EDIT', icon: 'fa-edit', href: route('follow-ups.edit', followUp.follow_up_id) },
                        followUp.status !== 'completed' && { label: 'DELETE', icon: 'fa-trash', onClick: deleteFollowUp, color: 'danger' },
