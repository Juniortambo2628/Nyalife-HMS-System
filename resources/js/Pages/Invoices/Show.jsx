...
export default function Show({ invoice, auth, clinic_settings = {}, paymentMethods = {} }) {
    ...
    const { data: voidData, setData: setVoidData, delete: destroyInvoice, processing: voidProcessing, reset: resetVoid } = useForm({
        void_reason: ''
    });

    const { data: paymentData, setData: setPaymentData, post: postPayment, processing: paymentProcessing, reset: resetPayment, errors: paymentErrors } = useForm({
    ...
    return (
        ...
        >
                    ...
                    <div className="col-lg-8">
                                        ...
                                        <tbody className="border-0">
                                            {invoice.items.map((item, index) => (
                                                <tr key={index} className="border-bottom border-gray-50">
                                                    ...
                                                    <td className="px-4 py-3 text-end text-muted small">{formatCurrency(item.unit_price)}</td>
                                                    <td className="px-4 py-3 text-center text-gray-700 fw-bold small">{item.quantity}</td>
                                                    <td className="px-4 py-3 text-end fw-extrabold text-gray-900">{formatCurrency(item.total_price)}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                                        ...
                                        <div className="p-4 rounded-4 bg-gray-50 border border-gray-100">
                                            <div className="space-y-3">
                                                <div className="d-flex justify-content-between align-items-center">
                                                    ...
                                                    <span className="fw-bold text-gray-800">{formatCurrency(subtotal)}</span>
                                                </div>
                                                {invoice.discount > 0 && (
                                                    <div className="d-flex justify-content-between align-items-center text-danger">
                                                        ...
                                                        <span className="fw-bold">- {formatCurrency(invoice.discount)}</span>
                                                    </div>
                                                )}
                                                <div className="d-flex justify-content-between align-items-center border-bottom border-gray-200 pb-3">
                                                    ...
                                                    <span className="fw-bold text-gray-800">{formatCurrency(taxAmount)}</span>
                                                </div>
                                                 <div className="d-flex justify-content-between align-items-center pt-2">
                                                     <span className="fw-extrabold text-gray-900">TOTAL DUE</span>
                                                     <span className="h4 fw-extrabold text-clinical-high mb-0">{formatCurrency(finalTotal)}</span>
                                                 </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div className="print-only mt-5 pt-5 border-top">
                            ...
                            <div className="text-center mt-8 opacity-25">
                                <p className="extra-small font-bold text-uppercase tracking-tightest">Printed on {formatDateTime(new Date())}</p>
                            </div>
                        </div>
                    </div>
                        ...
                        {payments.length > 0 && (
                            <div className="card shadow-sm border-0 mb-4 rounded-4 bg-white">
                                <div className="card-header bg-white py-3 border-bottom-0 pt-4 px-4">
                                    <h6 className="mb-0 fw-extrabold text-gray-400 extra-small text-uppercase tracking-widest">Payment History</h6>
                                </div>
                                <div className="card-body p-4 pt-0">
                                    <ul className="list-group list-group-flush">
                                        {payments.map((p) => (
                                            <li key={p.payment_id} className="list-group-item px-0 d-flex justify-content-between align-items-center">
                                                <div>
                                                    <Link href={route('payments.show', p.payment_id)} className="fw-bold text-success text-decoration-none">
                                                        {formatCurrency(p.amount)}
                                                    </Link>
                                                    <div className="extra-small text-muted">{p.payment_method_label} · {p.payment_date}</div>
                                                </div>
            ...
            <UnifiedToolbar 
                actions={[
                    !['paid', 'cancelled'].includes(invoice.status) && balanceDue > 0 && auth.user.role !== 'patient' && { 
                    ...
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content border-0 shadow-lg rounded-4">
                            ...
                            <div className="modal-body p-4 pt-0">
                                <p className="text-muted mb-4">Are you sure you want to void this invoice <strong>{invoice.invoice_number}</strong>? This action will mark it as voided but keep it in the system for auditing purposes.</p>
                        ...
                        <div className="modal-content border-0 shadow-lg rounded-4">
                            <form onSubmit={submitPayment}>
                                <div className="modal-header border-bottom-0 p-4">
                                    ...
                                    <button type="button" className="btn-close" onClick={() => { setIsPaymentModalOpen(false); resetPayment(); }}></button>
                                </div>
                                <div className="modal-body p-4 pt-0">
                                    <p className="text-muted small mb-4">
                                        Invoice <strong>{invoice.invoice_number}</strong> — balance due: <strong>{formatCurrency(balanceDue)}</strong>
                                    </p>
                                    <div className="mb-3">
                                        <label className="form-label fw-bold text-muted extra-small text-uppercase">Amount (Ksh)</label>
                                        <input type="number" step="0.01" min="0.01" className={`form-control ${paymentErrors.amount ? 'is-invalid' : ''}`}
                                            value={paymentData.amount} onChange={e => setPaymentData('amount', e.target.value)} required />
                                        {paymentErrors.amount && <div className="invalid-feedback d-block">{paymentErrors.amount}</div>}
                                    </div>
                                    <div className="mb-3">
                                        <label className="form-label fw-bold text-muted extra-small text-uppercase">Payment Method</label>
                                        <select className="form-select" value={paymentData.payment_method} onChange={e => setPaymentData('payment_method', e.target.value)}>
                                            {methodOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                                        </select>
                                    </div>
                                    <div className="mb-3">
                                        <label className="form-label fw-bold text-muted extra-small text-uppercase">Date</label>
                                        <input type="datetime-local" className="form-control" value={paymentData.payment_date}
                                            onChange={e => setPaymentData('payment_date', e.target.value)} required />
                                    </div>
                                    <div className="mb-3">
                                        <label className="form-label fw-bold text-muted extra-small text-uppercase">Reference (optional)</label>
                                        <input type="text" className="form-control" placeholder="M-Pesa code, etc."
                                            value={paymentData.transaction_reference} onChange={e => setPaymentData('transaction_reference', e.target.value)} />
                                    </div>
                                </div>
                                <div className="modal-footer border-top-0 p-4">
                                    <button type="button" className="btn btn-light rounded-pill px-4 fw-bold" onClick={() => { setIsPaymentModalOpen(false); resetPayment(); }}>Cancel</button>
                                    <button type="submit" className="btn btn-success rounded-pill px-4 fw-bold shadow-sm" disabled={paymentProcessing}>
