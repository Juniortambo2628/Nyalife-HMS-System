...
export default function Show({ patient, clinical_summary, auth }) {
    ...
    const { data: voidData, setData: setVoidData, delete: destroyVital, processing: voidProcessing, reset: resetVoid } = useForm({
        void_reason: '',
    });
    ...
    const consultationColumns = useMemo(() => [
        ...
        },
        {
            ...
            cell: ({ row }) => <TableCellPrimary>{formatDateTime(row.original.consultation_date)}</TableCellPrimary>,
        },
        {
            header: 'Physician',
            id: 'doctor',
            cell: ({ row }) => (
                <TableCellPrimary>
                    Dr. {row.original.doctor?.user?.last_name || 'Staff'}
                </TableCellPrimary>
            ),
        },
        {
    ...
    const appointmentColumns = useMemo(() => [
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <TableActions actions={[
                    { icon: 'fa-eye', label: 'View appointment', href: route('appointments.show', row.original.appointment_id) },
                ]} />
            ),
        },
    ], []);
    ...
    const prescriptionColumns = useMemo(() => [
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <TableActions actions={[
                    { icon: 'fa-eye', label: 'View prescription', href: route('prescriptions.show', row.original.prescription_id) },
                ]} />
            ),
        },
    ], []);
    ...
    const vitalColumns = useMemo(() => [
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <TableCellStack
                    primary={[row.original.temperature && `${row.original.temperature}°C`, row.original.heart_rate && `${row.original.heart_rate} bpm`].filter(Boolean).join(' · ') || '—'}
                    secondary={[row.original.respiratory_rate && `RR ${row.original.respiratory_rate}`, row.original.oxygen_saturation && `SpO₂ ${row.original.oxygen_saturation}%`].filter(Boolean).join(' · ')}
                />
            ),
        },
        {
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <TableActions actions={[
                    { icon: 'fa-edit', label: 'Edit vitals', href: route('vitals.edit', row.original.vital_id) },
                    { icon: 'fa-ban', label: 'Void record', color: 'danger', onClick: () => setVoidingVital(row.original) },
                ]} />
            ),
        },
    ], []);
            ...
            <div className="pb-5">
                ...
                <div className="row g-4">
                    ...
                    <div className="col-lg-4">
                        ...
                        >
                            <div className="nyl-detail-meta-row">
                                ...
                                <span className="fw-extrabold text-gray-900 small">{patient.emergency_name || 'Not specified'}</span>
                            </div>
                            <div className="nyl-detail-meta-row">
                        ...
                        {(patient.insurance_provider || patient.insurance_id) && (
                            ...
                            >
                                <div className="nyl-detail-meta-row">
                                    ...
                                    <span className="fw-extrabold text-gray-900 small">{patient.insurance_provider || '—'}</span>
                                </div>
                                <div className="nyl-detail-meta-row">
                    ...
                    <div className="col-lg-8">
                                ...
                                <div className="col-md-6">
                                    ...
                                    <div className="nyl-meta-grid">
                                        <div className="nyl-meta-item">
                                            ...
                                            <div className="nyl-meta-item__value">{patient.height ? `${patient.height} cm` : '—'}</div>
                                        </div>
                                        <div className="nyl-meta-item">
                                            <div className="nyl-meta-item__label">Weight</div>
                                            <div className="nyl-meta-item__value">{patient.weight ? `${patient.weight} kg` : '—'}</div>
                                        </div>
                                        <div className="nyl-meta-item">
                                        ...
                                        <div className="nyl-meta-item">
                                            ...
                                            <div className="nyl-meta-item__value">{canViewClinical ? prescriptions.length : 'Restricted'}</div>
                                        </div>
                                    </div>
                                </div>
                                <div className="col-md-6">
                                        ...
                                        {latestVital ? (
                                            <div className="nyl-meta-grid">
                                                <div className="nyl-meta-item">
                                                    ...
                                                    <div className="nyl-meta-item__value">{latestVital.blood_pressure || '—'}</div>
                                                </div>
                                                <div className="nyl-meta-item">
                                                    <div className="nyl-meta-item__label">Temperature</div>
                                                    <div className="nyl-meta-item__value">{latestVital.temperature ? `${latestVital.temperature}°C` : '—'}</div>
                                                </div>
                                                <div className="nyl-meta-item">
                                                    <div className="nyl-meta-item__label">Pulse</div>
                                                    <div className="nyl-meta-item__value">{latestVital.heart_rate ? `${latestVital.heart_rate} bpm` : '—'}</div>
                                                </div>
                                                <div className="nyl-meta-item">
                                                    <div className="nyl-meta-item__label">SpO₂</div>
                                                    <div className="nyl-meta-item__value">{latestVital.oxygen_saturation ? `${latestVital.oxygen_saturation}%` : '—'}</div>
                                                </div>
                                            </div>
                                        ) : (
                            ...
                            >
                                {clinical_summary?.consultation_date && (
                                    <p className="extra-small text-muted fw-bold mb-4">
                                        Sourced from consultation on {formatDateTime(clinical_summary.consultation_date)}
                                    </p>
                                )}
                        ...
                        {isReceptionist && (
                            <DashboardPanel title="Clinical records" icon="fa-lock" headerVariant="section" className="nyl-detail-panel" bodyClassName="p-4">
                                <p className="text-muted mb-0 small">
                                    Consultations, vitals, and pharmacy history are restricted to clinical staff.
                                    You can manage appointments and profile details from the toolbar.
                                </p>
                            </DashboardPanel>
                        )}
                    </div>
                        ...
                        <div className="modal-content border-0 shadow-lg rounded-4">
                            <div className="modal-header border-bottom-0 p-4">
                                ...
                                <button
                                    type="button"
                                    className="btn-close"
                                    onClick={() => { setVoidingVital(null); resetVoid(); }}
                                />
                            </div>
                            <div className="modal-body p-4 pt-0">
                                ...
                                <label className="form-label fw-bold text-muted extra-small text-uppercase">Reason</label>
                                <textarea
                            ...
                            <div className="modal-footer border-top-0 p-4">
                                <button type="button" className="btn btn-light rounded-pill px-4 fw-bold" onClick={() => { setVoidingVital(null); resetVoid(); }}>
                                    Cancel
                                </button>
                                <button
                                    type="button"
                                    ...
                                    onClick={() => {
                                        destroyVital(route('vitals.destroy', voidingVital.vital_id), {
                                            preserveScroll: true,
                                            onSuccess: () => { setVoidingVital(null); resetVoid(); },
                                        });
                                    }}
                                >
