...
export default function Laboratory({ requests, stats, filters }) {
    ...
    const columns = useMemo(() => [
        {
            ...
            cell: info => (
                <Link href={route('lab.show', info.getValue())} className="badge bg-light text-primary fw-bold text-decoration-none">
                    LAB-{info.getValue()}
                </Link>
            ),
        },
    ...
    return (
        <AuthenticatedLayout
            ...
            breadcrumbs={[{ label: 'Reports', url: route('reports.index') }, { label: 'Laboratory', active: true }]}
        >
            <Head title="Laboratory Report" />
            <ReportsNav active="laboratory" />
            <ReportDateFilter from={from} to={to} onFromChange={setFrom} onToChange={setTo} onApply={() => applyFilters()} exportType="laboratory" />
            ...
            <RegistryTablePanel title="Lab requests in period" icon="fa-flask" data={requests.data} columns={columns} pagination={requests} emptyMessage="No lab requests in this period." idField="request_id" />
        </AuthenticatedLayout>
    );
}
