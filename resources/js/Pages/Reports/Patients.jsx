...
export default function Patients({ patients, stats, filters }) {
    ...
    const columns = useMemo(() => [
        {
            ...
            id: 'name',
            cell: info => (
                <Link href={route('patients.show', info.row.original.patient_id)} className="fw-bold text-primary text-decoration-none">
                    {info.row.original.user?.first_name} {info.row.original.user?.last_name}
                </Link>
            ),
        },
        { header: 'Gender', accessorKey: 'gender', cell: info => <span className="text-capitalize">{info.getValue() || '—'}</span> },
        ...
        { header: 'Registered', accessorKey: 'created_at', cell: info => info.getValue()?.slice?.(0, 10) || info.getValue() },
    ], []);
    ...
    return (
        <AuthenticatedLayout
            ...
            breadcrumbs={[{ label: 'Reports', url: route('reports.index') }, { label: 'Patients', active: true }]}
        >
            <Head title="Patients Report" />
            <ReportsNav active="patients" />
            <ReportDateFilter from={from} to={to} onFromChange={setFrom} onToChange={setTo} onApply={() => applyFilters()} exportType="patients" />
            ...
            <div className="mb-3">
                <DashboardSelect
                    ...
                    value={gender}
                    onChange={(val) => { setGender(val || ''); applyFilters(val || ''); }}
                    placeholder="Filter by gender..."
                />
            </div>

            <RegistryTablePanel title="New patients in period" icon="fa-users" data={patients.data} columns={columns} pagination={patients} emptyMessage="No new patients in this period." idField="patient_id" />
        </AuthenticatedLayout>
    );
}
