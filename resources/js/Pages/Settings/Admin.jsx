...
export default function Admin({ stats, settings }) {
    ...
    const statItems = [
        ...
        { label: 'Pending prescriptions', value: stats.prescriptions_pending, icon: 'fa-prescription', color: 'danger' },
        { label: 'Newsletter subscribers', value: stats.newsletter_subscribers, icon: 'fa-envelope', color: 'secondary' },
    ];
                    ...
                    <div className="col-lg-8">
                        <div className="card border-0 shadow-sm rounded-3xl">
                            <div className="card-header bg-white border-0 pt-4 px-4">
                                ...
                                <p className="text-muted small mb-0">Read-only snapshot of stored settings. Edit landing page content via CMS.</p>
                            </div>
                                ...
                                {Object.keys(grouped).length === 0 ? (
                                    ...
                                    Object.entries(grouped).map(([group, items]) => (
                                        <div key={group} className="mb-4">
                                            <h6 className="extra-small fw-bold text-muted mb-3 text-capitalize">{group}</h6>
                                                    ...
                                                    <tbody>
                                                        {items.map((s) => (
                                                            <tr key={s.key}>
                                                                ...
                                                                <td className="small text-truncate" style={{ maxWidth: 280 }}>{s.value || '—'}</td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
                                                </table>
                        ...
                        <div className="card border-0 shadow-sm rounded-3xl">
                            <div className="card-body p-4">
                                ...
                                <div className="d-grid gap-2">
                                    <Link href={route('admin.api-tokens.index')} className="btn btn-outline-primary rounded-pill text-start">
                                        <i className="fas fa-key me-2"></i> API Tokens
                                    </Link>
                                    <Link href={route('cms.index')} className="btn btn-outline-primary rounded-pill text-start">
                                        <i className="fas fa-laptop-code me-2"></i> Landing Page CMS
                                    </Link>
                                    <Link href={route('users.index')} className="btn btn-outline-secondary rounded-pill text-start">
                                        <i className="fas fa-user-cog me-2"></i> User Management
                                    </Link>
                                    <Link href={route('reports.index')} className="btn btn-outline-secondary rounded-pill text-start">
                                        <i className="fas fa-chart-bar me-2"></i> Reports
                                    </Link>
                                    <Link href={route('admin.messages.index')} className="btn btn-outline-secondary rounded-pill text-start">
